...
- `.env` / `docker compose`
  ...
  - `DATABASE_VACUUM_ENABLED`
  - `DATABASE_VACUUM_INTERVAL_DAYS`
  - `DATABASE_VACUUM_TIME`
...
- `DATABASE_SYNCHRONOUS`：SQLite 同步级别，默认 `FULL`，更重视断电/强杀后的数据安全
- `DATABASE_BUSY_TIMEOUT_MS`：SQLite 忙等待时间，默认 `10000`
- `DATABASE_VACUUM_ENABLED`：自动维护时是否按计划执行数据库碎片整理，默认 `false`
- `DATABASE_VACUUM_INTERVAL_DAYS`：自动碎片整理间隔天数，默认 `7`
- `DATABASE_VACUUM_TIME`：自动碎片整理每日最早执行时间，默认 `03:30`
